...
import { useTenantUsersWithRole, useAddUserToTenantWithRole, useUpdateTenantUserRole, useRemoveUserFromTenant } from '@/hooks/use-tenants';
...
const ROLES = [
  ...
  { value: 'pm', label: '项目经理', color: 'bg-blue-100 text-blue-800' },
  { value: 'user', label: '普通用户', color: 'bg-green-100 text-green-800' },
];
...
export function TenantUsersDialog({ open, onClose, tenant }: TenantUsersDialogProps) {
  ...
  const availableUsers = allUsers?.filter((user: User) => 
    !(tenantUsers as TenantUser[])?.some((tenantUser: TenantUser) => tenantUser.id === user.id)
  ) || [];
          ...
          <div className="border rounded-lg">
            {isLoadingTenantUsers ? (
              ...
              <Table>
                ...
                <TableBody>
                  {(tenantUsers as TenantUser[]).map((user: TenantUser) => (
