...
import { updateUserProfile, updateUserPassword } from "@/lib/api/users";
import { updateStorageConfig, testStorageConfig } from "@/lib/api/settings";
import {
  SettingsHeader,
  StorageSettings,
  AccountSettings,
...
export default function Settings() {
  ...
  const handleSaveAccount = useCallback(async (config: AccountConfig) => {
    try {
      // 更新用户名
      if (config.username) {
        await updateUserProfile({ name: config.username });
      }
